...
local get_space_icons = function(windows)
	...
	for window in string.gmatch(windows, NEWLINE_REGEX) do
		...
		local app = (
			string.find(app_match, "Firefox") and "Firefox"
			or string.gsub(string.sub(app_match, 2, #app_match - 1), "^%s*(.-)%s*$", "%1")
		)
...
local update_previous_workspace_handler = function(env)
	...
	sbar.exec("aerospace list-windows --workspace " .. env.PREVIOUS_WORKSPACE, function(windows)
		...
		if not has_app then
			spaces[env.PREVIOUS_WORKSPACE] = nil
			sbar.remove("space." .. env.PREVIOUS_WORKSPACE)
			sbar.remove("space.padding." .. env.PREVIOUS_WORKSPACE)
			return
		end
